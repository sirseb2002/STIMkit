...
#Ceci dit d'appuyer sur le bouton 4 quand on est directement sur le bloc
def message():
    mc.postToChat("Veuillez appuyer sur le bouton 4")
...
def distance_now():
    ...
    while gameover == False:
        ...
        if (dist_now == 0) and (yd == 1):
            led_target.on()
            #Ceci dit d'appuyer sur le bouton 4 quand le joueur est sur le bloc
            if run_once == 0:
                message()
                run_once = 1
            if button4.is_pressed:
                print("Bouton 4 est appuyé")
                gameover = True
        else:
            led_target.off()
            run_once = 0
...
#Ceci dit au joueur d'appuyer sur le bouton 1
#Quand le bouton 1 est appuyé, cela fait apparaître le premier bloc au hazard
#Ensuite ceci dit au joueur d'aller trouver le bloc
if var_block1 == 0:
    mc.postToChat("Veuillez appuyer sur le bouton 1")
    button1.wait_for_press()
    print("Bouton 1 est appuyé")
    p = mc.player.getTilePos()
    x = p.x + randint(0, 25)
    y = p.y - 2
    z = p.z + randint(0, 25)
...
#Quand le bouton 2 est appuyé, cela fait apparaître le deuxième bloc au hazard
#Ensuite ceci dit au joueur d'aller trouver le bloc
if (var_block1 == 1) and (var_block2 == 0):
    mc.postToChat("Veuillez appuyer sur le bouton 2")
    button2.wait_for_press()
    print("Bouton 2 est appuyé")
    p = mc.player.getTilePos()
    x = p.x + randint(0, 25)
    y = p.y - 2
    z = p.z + randint(0, 25)
...
#Quand le bouton 3 est appuyé, cela fait apparaître le troisième bloc au hazard
#Ensuite ceci dit au joueur d'aller trouver le bloc
if (var_block1 == 1) and (var_block2 == 1) and (var_block3 == 0):
    mc.postToChat("Veuillez appuyer sur le bouton 3")
    button3.wait_for_press()
    print("Bouton 3 est appuyé")
    p = mc.player.getTilePos()
    x = p.x + randint(0, 25)
    y = p.y - 2
    z = p.z + randint(0, 25)
